...
async function cleanupDevFiles(projectDir: string, options: CleanupOptions): Promise<CleanupResult> {
  ...
  try {
    ...
    let templateIgnorePatterns: string[] = [];
    try {
...
async function processCleanupPatterns(
  projectDir: string, 
  patterns: string[], 
  whitelist: string[], 
  removedItems: string[]
...
async function downloadTemplate(ref: string, targetDir: string, framework: Framework): Promise<void> {
...
function resolveInputs(projectName: string | undefined, cliOptions: any): TemplateOptions {
  ...
  if (cliOptions.framework) {
    options.framework = cliOptions.framework as Framework;
